...
export const getNowTimeStr = (needTime) => {
  const date = new Date();
  if (needTime) {
    const nowTimeStr = `${date.getFullYear()}-${
      date.getMonth() + 1
    }-${date.getDay()} ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`;
    return nowTimeStr;
  }
  const nowDateStr = `${date.getFullYear()}-${
    date.getMonth() + 1
  }-${date.getDay()}`;
  return nowDateStr;
};
...
export const getPaths = (musicDir) => {
  ...
  const MUSIC_DIR = path.isAbsolute(musicDir)
    ? musicDir
    : path.join(__dirname, musicDir);
...
export const walkSync = (currentDirPath, callback) => {
  fs.readdirSync(currentDirPath, { withFileTypes: true }).forEach(function (
    ...
    const filePath = path.isAbsolute(dirent.name)
      ? dirent.name
